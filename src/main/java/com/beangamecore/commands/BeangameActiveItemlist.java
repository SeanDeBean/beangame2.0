...
public class BeangameActiveItemlist implements CommandExecutor{
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args){
        if(sender instanceof Player){
            ...
            if(player.getWorld().getName().equals("lobby") || player.isOp()){
                ...
                player.spigot().sendMessage(ChatMessageType.ACTION_BAR, TextComponent.fromLegacy("§cUse this command in the beangame lobby!"));
            }
        }
        return false;
    }
    public void init(){
        ArrayList<BeangameItem> items = new ArrayList<>(BeangameItemRegistry.getItemsInRotation());
        // elements
        ...
        for (int i = 0; i < Math.floor(items.size() / 45) + 1; i++) {
            Inventory inv = Bukkit.createInventory(null, 54, ChatColor.GOLD + "Beangame Active Itemlist!");
            for (int j = 0; j <= 44; j++) {
