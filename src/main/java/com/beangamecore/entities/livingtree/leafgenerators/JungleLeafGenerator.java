...
public class JungleLeafGenerator implements LeafGenerator {
    @Override
    public void generateLeaves(CanopyCenter center, TreeConfig config, List<BlockPosition> placedBlocks, Random random) {
        int radius = 5 + (config.getTreeHeight() / 3);
        int height = 6 + (config.getTreeHeight() / 4);
        LeafUtils.generateLeavesFromCenter(center, config, placedBlocks, radius, height, 0.35, "cylinder", random);
        LeafUtils.generateBranchEndClusters(center, config, placedBlocks, radius / 3, height / 3, 0.3, "sphere", 0.4, random);
    }
}
