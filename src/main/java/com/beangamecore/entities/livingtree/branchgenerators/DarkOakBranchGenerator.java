...
public class DarkOakBranchGenerator implements BranchGenerator {
    @Override
    public void generateBranches(TreeConfig config, List<BlockPosition> placedBlocks, Random random) {
        ...
        // More organic canopy with branching patterns instead of solid blocks
        for (int layerOffset = 0; layerOffset < 4; layerOffset++) {
            ...
            // Create branching pattern instead of solid canopy
    ...
    private void generateHorizontalBranch(int startX, int startY, int startZ, int dirX, int dirZ, 
        ...
        for (int i = 1; i <= length; i++) {
            ...
            // Add thickness for first few blocks
            if (i <= thickness) {
                // Add perpendicular thickness
                if (dirX != 0) { // Branch going east/west, add north/south thickness
                ...
                } else { // Branch going north/south, add east/west thickness
